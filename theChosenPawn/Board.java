...
public class Board {
    ...
    public static boolean noPiecesLeft(String color) {
        if (color == "White") {
            if (occupiedxw.size() == 0) {
                return true;
            } else {
                return false;
            }
        } else if (color == "Black") {
            if (occupiedxb.size() == 0) {
                return true;
            } else {
                return false;
            }
    ...
    public static boolean noMoreMoves(String color) {
        if (color == "White") {
            for (int i = 0; i < occupiedxw.size(); i++) {
                ...
                ArrayList<Integer> moves = tempPawn.loadMoves(occupiedxw.get(i), occupiedyw.get(i));
                if (moves.size() > 0) {
                    return false;
                }
            }
            ...
            for (int i = 0; i < occupiedxb.size(); i++) {
                Pawn tempPawn = new Pawn();
                ArrayList<Integer> moves = tempPawn.loadMoves(occupiedxb.get(i), occupiedyb.get(i));
                if (moves.size() > 0) {
                    return false;
                }
            }
